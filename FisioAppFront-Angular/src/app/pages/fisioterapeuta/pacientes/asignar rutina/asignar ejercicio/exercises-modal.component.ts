...
export class ExercisesModalComponent implements OnInit, OnDestroy {
  ...
  duplicarEjercicio(index: number) {
    const ejercicioOriginal = this.ejercicios[index];
    const ejercicioDuplicado: Ejercicio = {
      ...JSON.parse(JSON.stringify(ejercicioOriginal)),
      id: `ejercicio_${Date.now()}_${Math.random()}`
    };
    this.ejercicios.splice(index + 1, 0, ejercicioDuplicado);
  }
